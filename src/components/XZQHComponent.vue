    ...
    methods: {
        ...
        async getProvinceData() {
            ...
            const queryTask = new QueryTask({
                url: 'http://192.168.24.29:6080/arcgis/rest/services/Province/MapServer/0',
            });
            let query = new Query();
        ...
        async getCityAndCountyData(value) {
            ...
            const [QueryTask, Query] = await loadModules(['esri/tasks/QueryTask', 'esri/tasks/support/Query'], options);
            const queryTask = new QueryTask({
                url: 'http://192.168.24.29:6080/arcgis/rest/services/City/MapServer/0',
            });
            let query = new Query();
            query.returnGeometry = false;
            query.outFields = ['*'];
                ...
                Promise.all(
                    currentCityData.map(async (item2) => {
                        ...
                        const queryTask2 = new QueryTask({
                            url: 'http://192.168.24.29:6080/arcgis/rest/services/County/MapServer/0',
                        });
        ...
        async handleItemClick(val, type) {
            ...
            if (type === 'city') {
                ...
                serverUrl =
                    'http://192.168.24.29:6080/arcgis/rest/services/City/MapServer/0';
            } else if (type === 'county') {
                code = val.toString().substring(0, 6);
                serverUrl =
                    'http://192.168.24.29:6080/arcgis/rest/services/County/MapServer/0';
            }
